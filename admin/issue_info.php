...
<div id="main">
  ...
  <div class="container">
    ...
    $c=0;
      ...
      if(isset($_SESSION['login_user']))
      {
        $sql="SELECT student.username,roll,books.bid,name,authors,edition,issue,issue_book.return_b FROM student inner join issue_book ON student.username=issue_book.username inner join books ON issue_book.bid=books.bid WHERE issue_book.approve ='Yes' ORDER BY `issue_book`.`return_b` ASC";
      ...
      while($row=mysqli_fetch_assoc($res))
      {
        ...
        if($d > $row['return_b'])
        {
          ...
          mysqli_query($db,"UPDATE issue_book SET approve='$var' where `return_b`='$row[return_b]' and approve='Yes' limit $c;");
